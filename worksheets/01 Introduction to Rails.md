...
in your terminal, the first column shows the path prefixes Rails uses. If we append `_path` onto the end of the prefix this will be the name of the Rails helper method for that path. e.g. a prefix of `edit_product` will have a helper method of `edit_product_path` which will return the relative path to that route.
